...
typealias ProfileUrlTransformer = (tgt: R4Resource) -> Unit
...
class CareconnectTransformer(
    ...
    override fun transform(source: IBaseResource, target: IBaseResource): IBaseResource {
        ...
        return if (source is R3Bundle && target is R4Bundle) {
            // Transform the bundles profile url
            val bundleProfile = source.meta.profile
            bundleProfile.filter { profileMap.containsKey(it.valueAsString) }
            ...
            for (i in source.entry.indices) {
                ...
                // Transform each resource's profile urls
                val resourceProfile = sourceEntryResource.meta.profile
                resourceProfile.filter { profileMap.containsKey(it.valueAsString) }
                    .forEach { profileMap[it.valueAsString]?.invoke(targetEntryResource) }
            }
            target
        } else {
